...
import { ActivityType, Difficulty, LocationStatus } from '@prisma/client';
...
beforeAll(async () => {
  ...
  const near = await prisma.location.create({
    data: {
...
afterAll(async () => {
  await prisma.location.deleteMany({ where: { id: { in: [nearId, farId] } } });
});
