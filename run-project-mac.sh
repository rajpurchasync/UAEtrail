...
NODE_PACKAGE="node-v${NODE_BOOTSTRAP_VERSION}-${NODE_PLATFORM}-${NODE_ARCH}.tar.gz"
...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "This launcher is for macOS. Use ./run-project.sh on Linux/RHEL or run-project.bat on Windows."
  exit 1
fi
