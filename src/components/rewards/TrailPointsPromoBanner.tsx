...
export const TrailPointsPromoBanner = ({ variant = 'home', className = '' }: TrailPointsPromoBannerProps) => {
  ...
  return (
    <div className={outerClass}>
      ...
      >
        <div className={`relative flex items-center gap-3 ${padClass}`}>
          <span
            className={`flex shrink-0 ${isCompact ? 'h-10 w-10' : 'h-10 w-10 sm:h-12 sm:w-12'} items-center justify-center rounded-xl bg-white ring-1 ring-emerald-200/80 shadow-sm`}
          >
            <Trophy
              className={`${isCompact ? 'h-5 w-5' : 'h-5 w-5 sm:h-6 sm:w-6'} text-amber-500 fill-amber-400/20`}
              strokeWidth={2}
              aria-hidden
            />
          </span>
          ...
          <div className="min-w-0 flex-1">
            <p className="font-extrabold text-emerald-950 tracking-tight leading-snug text-sm sm:text-[15px]">
            ...
            <p className="hidden sm:block text-[13px] font-medium text-emerald-800/80 mt-0.5 leading-relaxed">
              {copy.line2}
            </p>
          </div>

          <span className="shrink-0 inline-flex items-center gap-0.5 text-xs font-bold text-emerald-700 group-hover:text-emerald-900">
